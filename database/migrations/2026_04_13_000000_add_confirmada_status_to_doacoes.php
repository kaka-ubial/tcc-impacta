        ...
        match (DB::getDriverName()) {
            ...
            'mysql' => DB::statement("
                ALTER TABLE doacoes
                    ...
                    DEFAULT 'pendente'
            "),
            default => null, // SQLite does not enforce enum constraints
        };
    }
        ...
        match (DB::getDriverName()) {
            'pgsql' => DB::statement("
                ALTER TABLE doacoes
                    DROP CONSTRAINT IF EXISTS doacoes_status_check,
                    ADD CONSTRAINT doacoes_status_check
                        CHECK (status IN ('pendente','entregue','cancelado','recusada'))
            "),
            'mysql' => DB::statement("
                ALTER TABLE doacoes
                    MODIFY COLUMN status ENUM('pendente','entregue','cancelado','recusada')
                    DEFAULT 'pendente'
            "),
            default => null,
        };
