...
export default function Profile({
    ...
    const [latitude, setLatitude] = useState<number | null>(
        auth.user.doador?.latitude ?? null,
    );
    const [longitude, setLongitude] = useState<number | null>(
        auth.user.doador?.longitude ?? null,
    );
    ...
    const [geocodingQuery, setGeocodingQuery] = useState(() => {
        ...
        return [e.logradouro, e.numero, e.cidade, e.uf, 'Brasil']
            .filter(Boolean)
            .join(', ');
    });
    ...
    function validateField(
        field: string,
        value: string,
        fieldRules: ((v: string) => string | null)[],
    ) {
                ...
                <div className="space-y-6">
                    {tipo === 'doador' && (
                        <div className="space-y-3">
                            ...
                            <DoadorFotoUploader
                                nome={
                                    auth.user.doador?.nome_completo ??
                                    auth.user.email
                                }
                                fotoAtual={
                                    auth.user.doador?.foto_perfil ?? null
                                }
                            />
                        </div>
                    )}
                                        ...
                                        <div className="flex flex-col gap-3">
                                            ...
                                            {selectedCausas.map((id) => (
                                                <input
                                                    key={id}
                                                    type="hidden"
                                                    name="causas_apoiadas[]"
                                                    value={id}
                                                />
                                            ))}
                                            <InputError
                                                message={
                                                    errors.causas_apoiadas as
                                                        | string
                                                        | undefined
                                                }
                                            />
                                    ...
                                    {tipo === 'doador' && (
                                        <div className="flex flex-col gap-3">
                                            <div>
                                                ...
                                                <p className="mt-0.5 text-xs text-muted-foreground">
                                                    O portal é público e lista
                                                    todas as doações entregues.
                                                    Por padrão você aparece como
                                                    doador anônimo.
                                                </p>
                                            </div>
                                            <label className="flex cursor-pointer items-start gap-3 rounded-lg border border-border bg-card px-3 py-2.5">
                                                ...
                                                <span className="text-xs font-medium text-foreground">
                                                    ...
                                                    nas doações que eu fizer
                                                </span>
                                            </label>
                                            <InputError
                                                message={
                                                    errors.exibir_em_transparencia as
                                                        | string
                                                        | undefined
                                                }
                                            />
